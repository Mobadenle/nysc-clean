...
// onGoSignup and onGoLanding are passed from App.jsx — no context routing
export default function LoginPage({ onGoSignup, onGoLanding }) {
  ...
  const handlePasswordLogin = async () => {
    ...
    try {
      ...
      // Auth state change in AuthContext will update session → App.jsx shows AppShell
    } catch (err) {
      ...
      <div className="auth-right">
        <div className="auth-form-card animate-in">
          ...
          <div style={{ textAlign: 'center', fontSize: 14, marginTop: 20 }}>
            ...
            <span className="auth-link" onClick={onGoSignup}>Create one free</span>
          </div>
          {onGoLanding && (
            <div style={{ textAlign: 'center', marginTop: 10 }}>
              <span className="auth-link" style={{ fontSize: 13, color: 'var(--text-muted)' }} onClick={onGoLanding}>
                ← Back to home
              </span>
            </div>
          )}
