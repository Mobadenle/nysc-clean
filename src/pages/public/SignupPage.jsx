...
export default function SignupPage({ onGoLogin }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      // If session is created immediately, onAuthStateChange fires → App shows AppShell
    } catch (err) {
  ...
  if (success) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
      <div style={{ textAlign: 'center', maxWidth: 440, padding: 32 }}>
        ...
        <button className="auth-link" style={{ marginTop: 24, fontSize: 14, display: 'block' }} onClick={onGoLogin}>
          Back to login
        </button>
      </div>
    </div>
