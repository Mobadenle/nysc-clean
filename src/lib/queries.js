...
export async function fetchIssues({ category, state, urgency, status, search, sortKey } = {}) {
  let query = supabase
    .from('issues')
    .select(`
  ...
  if (category && category !== 'All') query = query.eq('issue_categories.name', category)
...
export async function createResponse({ issueId, body, isAnonymous, isAmbassadorResponse, authorId }) {
  const { data, error } = await supabase
    .from('responses')
    .insert({
      issue_id:               issueId,
      ...
      is_ambassador_response: isAmbassadorResponse ?? false,
    })
    .select(`
      id, issue_id, body, is_anonymous, is_best_answer,
      is_ambassador_response, upvote_count, created_at,
